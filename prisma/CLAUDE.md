...
The same workflow then applies the real migration chain to a disposable database and runs three suites covering what `migrate diff` cannot see (it ignores objects it can't model): `catalog-schema-integration.mjs` (CHECK bodies, deferred lead triggers, `NULLS NOT DISTINCT`, audit immutability, the bootstrap recipes in this file), `staff-role-concurrency.mjs` (the last-admin race across the app and DB paths), and `catalog-addflow-integration.mjs` (the phase-2 add-flow, plus a **query-plan** assertion — the trigram predicate's wrong spellings return correct rows and merely lose the index, so nothing else can catch them).

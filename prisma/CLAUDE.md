...
⚠️ **OPEN DEFECT (2026-08-11): the fold functions carry NO `search_path` pin, so a write fails under a foreign path.** `catalog_fold_v1`'s body calls `f_unaccent(...)` unqualified with no `proconfig`, and `catalog_fold_arr_v1` calls `catalog_fold_v1` the same way. Because the fold backs five generated columns, `SET search_path = pg_catalog; INSERT INTO public.producers …` raises `function f_unaccent(text) does not exist` — measured. It fails CLOSED (a loud error, never a wrong fold), so it is availability, not correctness. ⚠️ `gtin_check_digit_ok` DOES pin `search_path`, so this is an inconsistency in our own migrations, not a convention. **The fix (`ALTER FUNCTION … SET search_path = pg_catalog, public`) changes the fold identity hash** (`proconfig` is in `pg_get_functiondef`: `47f391f0…` → `14a1792d…`) while leaving output byte-identical, so it is a **coordinated deploy with the catalog-maintenance side**, not a unilateral fix. Details + verification: `docs/dev/proposals/wine-catalog-implementation.md` § OPEN DEFECT.
...
✅ **`category` is constrained by the `categories` table** (`20260811215655`). Before it, the composite FK `(category, style)` → `category_styles` was the only guard — and it is **MATCH SIMPLE**, so it skipped its check whenever either column was null, and `style` is nullable. Measured on both `wines` and `wine_products`: `category='spirit', style='grappa'` rejected, `category='spirit', style=NULL` **accepted**. Same trap the `vintageId`/`productId` pair already guards against with an explicit CHECK, never applied here.

🔒 **Adding a category is an INSERT, not a migration**: one row in `categories`, then its styles in `category_styles`. ⚠️ **A `CHECK (category IN (SELECT …))` is not legal** — Postgres forbids a subquery in a CHECK — and `category_styles` can't be the FK target because its PK is the *pair*. Hence a table. `style` stays nullable on purpose: "known wine, unknown style" is a supported state, so `NOT NULL` on `style` was rejected as the fix. The two FKs are complementary — the composite one rejects a wrong style *for* a category, the single-column one rejects the category.

...
| `/api/auth/native/change-password` | 20/h/IP (atomic floor + BA) |
...
- **Native change-password**: brute-forces the CURRENT password from a stolen native session (a re-auth surface, like account PATCH). 20/h matches the web `rl:account` posture; keyed on IP (BA can't key on userId) via the atomic floor above. Same trusted-proxy IP posture.

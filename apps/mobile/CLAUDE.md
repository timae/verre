...
- **Aroma input (descriptor layer, PR B)**: `scoring/aroma/` — `AromaInput` (the 02e·11 search-first section on the impression screen: chips + browse/search row + suggestions + refine row; controlled `AromaSelection[]`, local-until-commit), `parts.tsx` (`AromaChip` — THE aroma badge everywhere: family-tinted via `useAromaColors`, no dots/ticks, border = Pronounced only; `ModifierRail`, `PronouncedRow`, `ModifierPopup` anchored refine, `useAromaOps` — every mutation re-gated + pair-precise, `canonicalPair` — re-target after promoted rewrites, `useTapOrDouble`), `SelectionSheet` ("+N more" overflow, cap-aware sizing), `BrowseSheet` (segmented four-picker control — ALL FOUR ship for device testing; Rail live, Rings/Map/Canvas pending), `RailPicker`. **Design spec-of-record: `docs/design/decisions/0008-aroma-input-search-first.md`** (variant pick, chip anatomy, Pronounced glyph/border split, ordering, add feedback, keyboard scroll) + `docs/dev/proposals/aroma/aroma-layer.md` (data semantics). Icons: `grid`, `pronounced` (reusable "standout" mark). `Button` gained `titleLines`; `SheetSearchField` gained `onFocus`/`onBlur`.

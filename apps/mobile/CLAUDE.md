...
- **Compare aromas (02d, §8/§9)**: `AromaCompareStrip` + `AromaDetailSheet` (`moments/`) over the ONE per-card `buildCompareAromaModel` from `moments/aromaCompareView.ts` — 🔒 CompareBody computes the model ONCE and passes it to BOTH; never let strip or sheet re-derive consensus/contributors themselves (the whole point of the model is that they can't drift). Every derivation is pure + pinned in `.local/test-env/scripts/aroma-aggregate-units.ts` (steps 38–38d): mode fork (agreement vs flat-union fallback, headers Group aroma / Aromas mentioned / Aromas), `detailPillColors` (per-theme contrast ladder — clay = solid rung), `shouldScrollAromaDetail` (combined-height, conservative 1 chip/line), `packStrip` (measured widths, always-present tail — deliberately separate from `packChips`, see its comment). Strip owns measurement (resets on fontScale+width+height; pre-measure row hidden + height-bounded) and popover state (open state carries the strip SIGNATURE so live-poll changes can't strand it). The "+ Aroma details" pill is a LOCAL `AromaDetailPill` — don't re-generalize `MoreChipsPill` for it. Consensus knob defaults are FROZEN in core (`DEFAULT_CONSENSUS_OPTS`: primary > n/2 · peak ≥ ⅓ · pronounced > n/2); the dev-gallery knob lab is the only place they vary. Slice 3d (detail-sheet tabs over the staged `Tier3Mode`) is the known next step. As-built record: `docs/dev/proposals/aroma/aroma-layer.md` §8.
- **Aroma READ**: `AromaReadChips` (`scoring/aroma/AromaReadChips.tsx`) — read-only grouped chips (displayOrder + AromaChip) for someone's stored selections; feed impression detail now, further native read surfaces reuse it. The `lead` word prop was REMOVED (fixes round 2026-07-13 — the feed detail carries a section title instead). Pass `collapse` for the rating page's overflow behaviour: two lines via the shared width packer (`packChips`/`ChipMeasurePass`/`MoreChipsPill` in `parts.tsx`, also used by `SelectedChipsRow`), tail behind a "+N more" pill that opens a read-only bottom sheet — the host screen needs a `BottomSheetModalProvider`. ⭐Read badges are COMPACT: `AromaReadChips` passes `vPad 0` (inner padding 0 + the inherited 23pt label line via `badgeVMetrics`; Simon 2026-07-13) to every chip + pill it renders — write surfaces keep the 4.5 default; don't override per call site. The WEB has its own twin (`components/ui/AromaReadChips.tsx`, PR D — no shared RN/web layer). No input, no refine — mutations only through AromaInput.

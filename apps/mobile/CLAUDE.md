...
- **Rating block (WRITE)**: `RatingSection` (`scoring/RatingSection.tsx`) — THE score · note · adaptive structure-fold (ⓘ intensity reference) · aromas section, shared VERBATIM by the moment impression screen (02e) and the standalone check-in rate stage (Simon 2026-07-12: one component so a future ordering/copy/behaviour change lands on both; the impression's anatomy — Score → Your Note → Structure fold → Aromas — is the spec). Fully controlled: screens own every value plus the fold-open flag (the impression seeds it open on structure engagement); pairs with `useAromaSearchScroll` (`scoring/aroma/useAromaSearchScroll.ts`), the shared screen-side keyboard-fit hook (screen provides scrollRef + a scrollY-tracking onScroll). Don't hand-compose these widgets on a new rating surface.

...
- ✅ **DONE — `<AnchoredMenu>`** (`ui/AnchoredMenu.tsx` + `elevation.menu` token; 3 panels migrated, reviewer-verified). ⏳ `add.PositionPicker` (`.at-pospop`) still hand-rolls the same shadow — fold it onto `elevation.menu` when next touching add.tsx; the remaining site a CI shadow-gate should cover.

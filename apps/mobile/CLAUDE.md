...
- ✅ **DONE (commit pending) — `<AnchoredMenu>`** extracted to `ui/AnchoredMenu.tsx` + the `elevation.menu` token; the 3 panels (IrMenu/SessionMenu/RowMenu) migrated, reviewer-verified behavior-preserving. ⏳ `add.PositionPicker` (`.at-pospop`) still hand-rolls the same shadow — fold it onto `elevation.menu` (and ideally `AnchoredMenu`-style) when next touching add.tsx; it's the remaining site a CI shadow-gate should cover.

...
- **Compare aromas (02d, §8/§9)**: `AromaCompareStrip` + `AromaDetailSheet` (`moments/`) over the ONE per-card `buildCompareAromaModel` from `moments/aromaCompareView.ts` — 🔒 CompareBody builds the model only while that card/detail sheet is open, memoized by `compareAromaInputSignature`, and passes it to every compare-aroma surface; never let a renderer re-derive consensus/contributors itself. Unknown-only historical aroma ids do not admit a dead card (`hasResolvableAroma`). Aggregate counts + contributor identities share core `aromaAncestorChain`; the model derives ONE `pronouncedIds` set consumed by strip + sheet. `model.bun` is the FULL counted consensus tree projection (context/primary/secondary/peak; headings excluded), never the two-line strip: **Aroma Details → Agreement** switches between Tree and Aroma Bun, and the Bun's own physical label-fit is its only display cut. Every derivation is pure + pinned in `.local/test-env/scripts/aroma-aggregate-units.ts` (steps 37b–38d): mode fork (agreement vs flat-union fallback, headers Group aroma / Aromas mentioned / Aromas), `detailPillColors` (per-theme contrast ladder — clay = solid rung), `packStrip` (measured widths, always-present tail — deliberately separate from `packChips`, see its comment). Strip width keys include id + live count + pronounced state; an open popover remeasures its trigger after renders so poll reflow cannot strand it. `AromaDetailSheet` is a FIXED 85% scroll sheet across every tab — never dynamic-size it per tab. Tab order is Agreement / People / All Aromas when agreement exists, otherwise People / All Aromas; switching tabs must not move the sheet. The detail sheet contains: Agreement (Tree/Aroma Bun; agreement mode only) / **All Aromas** (compact Mentions/Family switch on the LEFT and the standard `SheetSearchField` filling the RIGHT of the SAME ROW—the app-wide toolbar order; Mentions = one FLAT occurrence-ranked field of exact canonical `AromaChip` pairs, so modifiers remain inside the badge exactly like impression detail; Family = the SAME badges grouped under a family dot/header, ranked by each family's summed literal family/subfamily/leaf mentions, with occurrence-ranked badges inside; taxonomy order only breaks ties; every badge count is distinct tasters for that exact pair; no base-heading/modifier-tally analytics) / **People** (search matches BOTH names and aroma vocabulary, including a family query such as Fruity matching descendant picks; within every matching person, matching badges move FIRST and nonmatches use the same `muted` paling as AromaInput search; established Moments/Compare row anatomy with comfort-scaled 40pt Avatar + name + exact `AromaReadChips`, roster order, two-line collapse; "+N more" PUSHES the established full-badge read sheet titled in Title Case: "`<Name> Perceived N Aromas`"). `AromaBadgePopover` is the ONE ruled Badge-Extension shell: Tier-2, All-Aromas badges, and People badges all reuse it; it renders OUTSIDE the gorhom sheet tree so opening it can never alter sheet measurement. A People badge says "Also perceived by" and excludes its row owner; the rejected duplicate inline "Perceived by" block under All Aromas stays removed. People also shows `aromaTasteSummary`: weighted taxonomy Dice (family 1, subfamily 2, leaf 3, explicit modifier 1; Pronounced excluded), producing Most alike / Most different / Closest to group / Most individual from THREE aroma respondents up (fewer → the panel is suppressed: with two, every stat names the same pair — Simon 2026-07-17); compute it only while the sheet is mounted, never in every card poll. Every stat card TAPS THROUGH (Simon 2026-07-17) to a pushed 70% ranking sheet (`TasteDetailSheet` — sheet-over-sheet is the ruled navigation; an in-sheet drill was rejected as illegible): top/bottom 8 pairs or people with scores, #1 expanded, single tap-to-swap expansion; expanded pairs show the `tasteSharedEvidence` chips (SHARED = exact picks + same-leaf, RELATED = subfamily/family territory not covered by a finer shared node), expanded people show their closest matches; a footer names the cut (`Top 8 of N pairs`). `summary.pairs`/`summary.group` are the complete stable-sorted rankings; `aromaTasteSummary` is computed ONCE at the CARD (CompareBody), gated on a sheet needing it, and passed to both sheets. 🔒 The ranking sheet MOUNTS AT THE CARD as the detail sheet's SIBLING (screen-level sheet anatomy), never inside the conditionally-mounted AromaDetailSheet — the first cut nested it there and closing it stranded the detail sheet presented-but-closed on device (invisible, blocked reopening, flickered on unmount). Each tab owns its own `compareSelectionReducer` state and search state: switching views preserves that tab's focus for return but never carries muting/highlighting into another tab. Within a tab, selection is single-select aroma XOR participant and retap clears; aroma identity remains the THREE-WAY `AromaRef` because popovers/routes still distinguish node = subsumed, base = literal-all-modifiers, pair = exact. All Aromas taps are pair refs. 🔒 On Agreement only a NODE selection accents a consensus node + shows names; base/pair selections just dim to supported branches. 🔒 `peopleFilter` is separate navigation context, set by popover routes and cleared only by Show All ✕; search filters within that context and selecting a person never clears it. Active tab + filter + each tab-local selection are validity-derived every render; tab changes reset only the shared scroll viewport. The "+ Aroma Details" pill remains the local `AromaDetailPill`. Consensus defaults are frozen in core (primary > n/2 · peak ≥ ⅓ · pronounced > n/2). As-built record: `docs/dev/proposals/aroma/aroma-layer.md` §8.

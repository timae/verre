...
- **Aroma READ**: `AromaReadChips` (`scoring/aroma/AromaReadChips.tsx`) — read-only grouped chips (displayOrder + AromaChip, optional lead word) for someone's stored selections; feed impression detail now, profile + further read surfaces reuse it (PR D). No input, no refine — mutations only through AromaInput.

...
import type { MySessionRow } from '@/lib/api/sessions';
...
// ['my-sessions'] cache (the Moments tab list) — which by construction contains
...
export function useEnterableMoment(sessionId: number | null | undefined): {
  ...
  // Read the cache directly (no new fetch) — the Moments tab already warmed it.
  const rows = queryClient.getQueryData<{ rows: MySessionRow[] }>(['my-sessions'])?.rows;

...
- `searchIndex()` — **PR B, not yet built** (the `search_aliases` + `modifier_display` data it consumes is already in the JSON): flat index over node labels (all tiers, since #6) + modifier labels + modifier `search_aliases` + `modifier_display` words, so "dried fig" resolves to `fig + dried` and "jammy strawberry" to `strawberry + cooked`. ⚠️ Four subfamily labels equal leaf labels (`honey`, `vanilla`, `cocoa`, `char` — e.g. `sweet.honey` vs the honey leaf): ids stay unambiguous, but search/UX must disambiguate the two selectable nodes, and mixed-grain counting (compare milestone) should treat an eponymous coarse-vs-leaf pick as near-duplicate.

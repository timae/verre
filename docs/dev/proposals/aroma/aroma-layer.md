...
```
{ a: <node id — any tier>, m: <modifier id> | null, d?: true }
```
...
- `d` — **dominant flag (Simon, 2026-07-08)**: marks the note(s) that led the impression. Optional boolean on the wire; canonical/stored form carries it only when `true` (absent = not dominant, `false` normalized away). Not part of the dedupe identity (duplicate `(a, m)` pairs merge, dominant wins) and never an extra cap unit. Server puts NO ceiling on how many selections are dominant — if emphasis inflation becomes a problem it's a client-UX concern, not a validation rule. The user-facing word ("Dominant" vs "Standout" vs …) is UI copy for PR B.
...
- **Redis** (`s:{CODE}:r:{ID}:{WINEID}` JSON): add `aromas: [{a, m, d?}]` beside `score`/`flavors`/`notes`. Absent/empty array = none.
...
- **core `validateAromaSelections(input)`** — shape check: array, ≤ cap, each element `{a: string, m: string|null, d?: boolean}` (`d` normalized to present-only-when-true), dedupe. Shared with native for optimistic state.

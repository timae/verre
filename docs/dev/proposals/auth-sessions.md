...
- **geo is SHIPPED and country-only** — NOT the city-level / IP-anonymisation design the body below (§4) describes. As built: an in-process seek-based lookup (`lib/geo.ts`) over binary tables generated from public RIR delegated-stats (public domain, no licence/account), delivered via the deploy job → S3 → boot-download (`scripts/generate-geo-table.mjs`, `scripts/refresh-geo-data.mjs`, `lib/geoData.ts`, `instrumentation.ts`; see `docs/dev/deployment.md`). The IP is resolved to a country name ("Switzerland") fully on-server and never sent anywhere; NO IP anonymisation (there's no third party to protect from, and the raw IP is never persisted). The §4 text below ("last octet zeroed", "Zurich, CH", "stub") is the original plan, superseded. The 90-day revoked-row cleanup is still deferred.

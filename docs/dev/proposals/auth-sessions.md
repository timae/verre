...
- **geo is SHIPPED and country-only** — NOT the city-level / IP-anonymisation design the body below (§4) describes. As built: an in-process seek-based lookup (`lib/geo.ts`) over binary tables generated from public RIR delegated-stats (public domain, no licence/account). Delivery: a **weekly scheduled job** refreshes the tables into S3; the app downloads them at boot (`ensureGeoData` in `lib/geoData.ts` via `instrumentation.ts`), and background-seeds S3 on a cold start. (`scripts/generate-geo-table.mjs`, `scripts/refresh-geo-data.mjs`; see `docs/dev/deployment.md`.) The IP is resolved to a country name ("Switzerland") fully on-server and never sent anywhere; NO IP anonymisation (no third party to protect from, raw IP never persisted). The §4 text below ("last octet zeroed", "Zurich, CH", "stub") is the original plan, superseded.
- **90-day revoked-row cleanup is SHIPPED.** pg_cron isn't on Nine's managed Postgres, but Deplo.io has native `scheduledJobs` (cron) — so a **daily scheduled job** prunes `user_sessions` revoked >90 days via `prisma db execute --file prisma/maintenance/cleanup-revoked-sessions.sql`. Real 90-day retention floor (not deploy-cadence-dependent); a job failure never affects the release. See `.deploio.yaml` + `prisma/maintenance/cleanup-revoked-sessions.sql`. (§11's "bounded-retention design" deferred item — now built.)

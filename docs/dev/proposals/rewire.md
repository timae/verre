...
- **Pre-cutover live-session check**: confirm no session is currently active (no participant has rated within the last hour, no host is mid-engagement). Scaling the app to 0 mid-tasting bounces participants. At Tim+Simon scale just text each other; future: query Redis for any `s:*:r:*` key touched recently.

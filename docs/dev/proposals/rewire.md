...
- **Refactor the rate endpoint away from `wineId_userId` compound-key reliance.** Today `app/api/session/[code]/rate/route.ts:78,84` use `prisma.rating.findUnique({where: {wineId_userId: ...}})` and `upsert({where: {wineId_userId: ...}})`. Both names are Prisma-generated from the existing `@@unique([wineId, userId])`. Replace with `findFirst({where: {wineId, userId}})` + a manual upsert (find → if found update, else insert). Behaviour unchanged.
- **Drop `@@unique([wineId, userId])` on `ratings`. Add raw partial unique on `(user_id, wine_id, session_id) WHERE session_id IS NOT NULL AND user_id IS NOT NULL`** (the `user_id IS NOT NULL` clause is load-bearing — Postgres treats NULLs as distinct, so without it anon two-tab races wouldn't be constrained). Make `ratings.score` nullable. The rate-endpoint refactor above is the prerequisite that makes this swap safe.
- **Persist `wines.revealedAt` to Postgres on every reveal endpoint** (`/api/session/[code]/wines/[wineId]/reveal` POST + DELETE, `/api/session/[code]/wines/reveal-all`, `/api/session/[code]/wines/hide-all`). Each gains a Postgres `UPDATE wines SET revealed_at = $value WHERE id = $wid` (or `WHERE session_id = $sid AND revealed_at IS NOT NULL` for hide-all). Today these endpoints only touch Redis; this prereq is what lets phase 2's renderer read `wines.revealedAt` from a stable source post-Redis-eviction. Pre-existing condition worth flagging: `pgUpsertWine` only fires for logged-in hosts (the wine row only exists in Postgres if the session host was logged in at wine-add time). The new UPDATE silently no-ops for anon-host wines — acceptable today (anon-host blind sessions are rare; the renderer falls back to Redis while it's hot, and post-eviction the wines render redacted, which is the safe default).

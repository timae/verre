...
- **Like reconcile** — mobile now applies the server's authoritative `{ liked, count }` response after the optimistic ±1 (idempotent double-likes + block-pair-hidden likes make the real count diverge; web already did this).

...
- `08` feed — 📋 PROPOSED (2026-07-06): native feed over the shipped `/api/feed` backend. Design = 03·12 "linked · glass" with three deviations (no fullscreen-from-feed; glass panel opens a full PAGE not a sheet; double-tap-to-like) + a NEW read-only impression detail screen (not 02e). Standalone-card redesign is Phase 2. Not started

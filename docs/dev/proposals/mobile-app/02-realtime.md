...
- **Logto's token refresh is part of this hang surface — wrap it in YOUR OWN helper, the SDK gives you no timeout hook.** `@logto/rn` refreshes lazily on `getAccessToken()`, no background loop (iOS suspends JS). So the *first authed call* on cold-start/resume may need a Logto refresh round-trip before it can hit Verre's API, and it lives *inside the SDK*. The mitigation is a hand-written `getValidToken()` wrapper around `getAccessToken()` that (a) holds a module-level in-flight promise so concurrent callers share one refresh (**single-flight**), and (b) races it against your own timeout. **Important nuance:** `getAccessToken()` takes no `AbortSignal`, so this is a **soft timeout** — you stop *waiting* and show a reconnecting state, but the SDK's underlying refresh keeps running. (Don't conflate this with the `/state` poll, which IS abortable via `AbortController`.) And: when the shared refresh promise *rejects* (refresh genuinely failed), every queued caller must see the rejection and the app routes to **re-login** — not a silent retry-storm. Also configure Logto's **reuse-detection leeway**: a backgrounded-then-resumed instance retrying with a rotated-but-lost refresh token trips reuse-detection and **silently logs the user out mid-session** — a real Logto incident class (rotation is forced for public clients); single-flight covers it.
- **The real gap is reconnection robustness on bad signal** (cellars, restaurants). The plan must add, and the original draft omitted: **request timeouts** for the `/state` poll (RN `fetch` has *no* default timeout — a poll into a dead radio hangs ~30–60s on TCP; wire `AbortController` — this one IS abortable, unlike the Logto refresh above), **network-state awareness** (`@react-native-community/netinfo` → TanStack `onlineManager`, plus a "reconnecting" affordance), and a **single-flight guard** so a foreground-resume request storm doesn't fire N parallel refetches into a flaky reconnect.

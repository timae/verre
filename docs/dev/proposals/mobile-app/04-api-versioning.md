...
## 4. Recommendation: versioning co-arises with the Logto split, and the client screen ships in build #1

The original "maybe not yet" framing is **superseded** — but precisely. The auth *routing* fork keys on the **bearer token's presence**, not the version header (`01` §5: "`Authorization` present ⇒ Logto authoritative"), so versioning is **not a literal prerequisite** of the split. Rather: the Logto split is the first thing that **creates a second, independently-deployed client generation**, and *that* is what makes versioning load-bearing (response-shape decisions + a force-update floor once a client you can't redeploy exists). So they **co-arise** — ship the version boundary alongside the split, not as a gate the split waits on. What's still right-sized vs over-built:

- **Now (prerequisite)**: establish the version boundary — **send `X-Verre-Client` (native version + OTA id, §3a) from day one**, and define how the API distinguishes client generations. This is what the Logto/NextAuth split rides on.

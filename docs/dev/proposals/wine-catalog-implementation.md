...
- 🔒 **A `rejected` resolution is terminal and must be persisted as such.** The three staff resolutions are not symmetric: re-point and purge *change the data*, so the next retry simply succeeds — but **rejecting the proposed EAN leaves the conflict exactly as it was**, so a naive implementation returns `deferred` forever and the caller can never advance. So a rejection is stored as a durable terminal verdict on `(ean, incomingProductId)`, and subsequent retries return an **acknowledged rejection** — distinct from `deferred` — which tells the caller to advance its journal *without* attaching that EAN. Re-proposing the same pairing later is a no-op, not a fresh review item; a staff member can clear the verdict if they change their mind. ⚠️ **But the clear alone does NOT reopen the case, and this line is quoted in isolation often enough to say so here:** clearing changes nothing the *sending* side can observe — a rejection advances the product's content hash, so unchanged source data yields no diff and the product is never re-sent, and while a verdict stands a re-proposal is a no-op by definition. Reconsideration therefore needs the clear **plus** the verdict-cleared delivery defined in the next bullet. That delivery is agreed-in-design and unbuilt (phase-4 journal; phase 4 unstarted), so **the trap is open on both sides today**.
...
1. ✅ **SHIPPED (PR #93, 2026-07-27) — the first-catalog-fill blocker is CLEARED.** Built on both surfaces with five CI suites; the as-built deltas and corrections are recorded in the sub-bullets below. Kept in full because the *rationale* is the rationale-of-record, and because several sub-bullets are standing rules a later change can violate. **Legal attributions surface — 🔒 WAS A HARD GATE ON THE FIRST CATALOG FILL, and it is a LICENCE OBLIGATION, not a courtesy.** A config-driven attributions/legal page, live in **both** the web app and the native app, rendering entries supplied as **deploy configuration** rather than hardcoded. Some licences the catalog relies on **legally require naming the source**, so shipping catalog data without this surface is a licence breach — which is what makes it a hard release gate on the initial seed rather than a phase-4 nicety.
   - 🔒 **Attribution is CORPUS-level, never per-record.** Naming sources on this surface satisfies the licence; attaching a source id/URL/slug to a catalog row does not, and is separately forbidden (RFC § Data-provenance rule, as scope-corrected). The config is a flat list describing the CORPUS, with no join back to individual rows.
   - ⚠️ **The config shape recorded here was `{ source, licence, url }` and was WRONG** — corrected during implementation, ✅ **SHIPPED**. Two independent constraints broke it: **MIT** requires its permission notice reproduced VERBATIM, so `licence` cannot be a one-line string (it is a text BLOCK); and an entry must be able to say its wording is UNVERIFIED, or a best-guess paraphrase silently becomes the record. As-built shape (`lib/attributions.ts`): `{ source, sourceUrl, licence: { spdx, url, text }, attribution, verified, notes, dataPeriod? }`.
   - 🔒 **`sourceUrl` and `licence.url` are SEPARATE fields, and the split is a compliance fix, not tidiness.** A single `url` conflated "where the dataset lives" with "where the licence lives" — which left the LWIN entry's only clickable link pointing at a general dataset page while **CC BY explicitly requires "provide a link to the license"**. Liv-ex publishes a dedicated licence page (`liv-ex.com/lwin-creative-commons-licence/`, verified firsthand 2026-07-26). Both links render, each labelled for what it is; `licence.url` is the compliance-relevant one and is validated + parity-checked.
   - 🔒 **`licence.text` is a `scrub()` CARVE-OUT** — rendered verbatim, never normalized, never trimmed. Measured against the real `lib/textSafe.ts`: `scrub` PRESERVES the OGL-BC en dash (U+2013 sits below `SCRUB_RE`'s range) but its `.trim()` DROPS the MIT block's trailing newline (1072 → 1071 bytes). The house rule "apply `scrub` to every free-text field" (`lib/CLAUDE.md`) is about untrusted request bodies and does not apply here — but it is the tool someone will reach for, so the carve-out is stated explicitly in the module and asserted in CI.
   - 🔒 **The OGL-BC en dash is load-bearing.** Read firsthand from `www2.gov.bc.ca` on 2026-07-26 (the source was reachable from this sandbox, though not from the data side's): the REQUIRED statement uses an EN DASH — `Contains information licensed under the Open Government Licence – British Columbia.` — while the same page's own TITLE uses an ASCII hyphen. Both forms appear in one document, so a well-meaning punctuation normalisation would alter a legally required string.
   - **Both surfaces built.** Web renders server-side at `/legal/attributions`; the native app CANNOT read server env, so it fetches `GET /api/legal/attributions` (public, unauthenticated — a legal surface must render for a signed-out user on first launch) and falls back to a bundled snapshot when offline. 🔒 **The fallback's cost is staleness, and staleness is rendered visibly** — two copies of legal text can disagree, and the bundled one must never masquerade as current.
   - 🔒 **Reachable SIGNED OUT on both surfaces, which is the point of the public route.** Native: `about.tsx` + `attributions.tsx` live at the app ROOT, outside `<Stack.Protected>` (same placement rationale as `update-required`) — inside `(tabs)` they were session-gated and unreachable exactly when someone evaluating the app wants to read them. Entry points: the auth landing screen when signed out, You → About when signed in. Web: the lobby footer (signed out) and the `/me` dashboard (signed in — deliberately OUTSIDE the `show('quick_links')` preference gate, since a legal surface must not be user-hideable).
   - ⚠️ **`networkMode: 'always'` + `refetchOnReconnect: 'always'` on the native query are load-bearing.** TanStack defaults to `networkMode: 'online'` and `lib/query.tsx` wires NetInfo into `onlineManager`, so by default the query is PAUSED while offline and `fetchAttributions` never runs to return its fallback — a spinner forever, in precisely the case the fallback exists to serve. And `'always'` disables the default reconnect refetch, so the second option is required alongside it; the literal `'always'` rather than `true`, because a bundled result is fresh for an hour and `true` would honour `staleTime` and skip the refetch. Pinned by `scripts/tests/attributions-offline-units.ts` (both deletions verified to fail it).
   - 🔒 **The two copies are CI-gated** (`scripts/check-attributions-parity.mjs`, workflow `check-attributions-parity`): every legally-meaningful field compared BYTE-FOR-BYTE, plus assertions that no `verified: false` entry ships and that the en dash survives. Verified to FAIL against three real mutations (en dash → hyphen, MIT trailing newline trimmed, an entry flipped to unverified). ⚠️ Its first cut passed while parsing ZERO entries — the type annotation's `[` was matched as the array literal — so it now fails loudly on an empty parse rather than reporting green while checking nothing.
   - 🔒 **DO NOT trim the list by "which sources actually contributed rows".** A source can contribute facts via enrichment while contributing no rows, and the STRICTEST obligation (MIT) attaches to the source with the LEAST data.
   - **Known gap (recorded, not closed):** OGL-BC binds to the licence version in force as of the date the data was ACCESSED, and that access instant was never measured. Retrieval is bounded between **2026-07-15** (BC's own resource `last_modified`) and **2026-07-24** (repo capture) — narrow enough that a version change inside it is implausible rather than unknown. ⚠️ Deliberately NOT rendered as an access date: our stored timestamps resolve to repo-capture (a git-add date, identical across all inputs because they were committed together). What IS rendered is the **data period, April 2026** — confirmed at source by the data side (BC's resource is named `BC_Liquor_Store_Product_Price_List_April_2026`, our copy is byte-identical to what BC serves today, and that edition is still current).
   - Because the entries are deploy configuration, adding or changing a source is an ops change, not a release — which is the point, since licence terms can change independently of the app. ⚠️ **As-built qualifier (do not drop it, and do not write "immediately" or "always current"):** THREE deliberate caches sit between an ops edit and a native screen — the server process-caches the parsed file (so even the **web** needs a process restart), `GET /api/legal/attributions` sends `max-age=3600` + `stale-while-revalidate`, and the native client holds a LIVE result fresh for an hour. The accurate contract: **the web reflects the running server's snapshot; online native refreshes subject to its client and API caches; a failed fetch falls back to the release-time bundle compiled into the binary and links to the server-published web page** (Simon's ruling, 2026-07-26). Only a native release changes that bundle. Within one app session a previously-fetched live value may persist instead of the bundle — the query cache is in-memory with no persister, so a cold launch while offline gets the bundle.
   - 🔒 **An override is a COMPLETE REPLACEMENT and is validated as one.** `ATTRIBUTIONS_PATH` swaps the whole list, so a file with one valid new source would otherwise DELETE the four mandatory ones. The loader rejects an override that omits a bundled source, repeats a source, carries `verified: false`, has blank notes, or uses a non-`http(s)` URL — atomically, never merging a partial override into the bundled list. This closes a hole the CI parity gate structurally cannot see, since the gate only compares the two in-repo copies and never a file supplied at runtime.

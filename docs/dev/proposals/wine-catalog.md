...
- The shapes of **`producers`, `wine_products`, `wine_vintages`, `product_producers`, `product_eans`** (plus the `wines.productId`/`wines.vintageId` link columns) are **part of the versioned import contract**. Any change to these shapes is a contract change: surfaced explicitly and shipped with a contract version bump — never made silently.
  ...
  **Consequence for anyone reading this later: the audit is not a partial backstop for the replay — it is a non-backstop that looks like one.** It answers "can this constraint fire at all?", never "does it fire where the migrations say it should." Two sides reached that conclusion by different routes (FK masking here, no masking there), which is what makes it the property rather than either observation.
  ...
  ✅ **OUR half is now gated too** (`STYLE-VOCABULARY-CHANGED`, 2026-08-12). The snapshot records the `category_styles` pairs, and any change — addition, removal, relabel — fails the gate with the ordering stated in the failure message.

  🔒 **EACH SIDE HAD INSTRUMENTED THE OTHER'S HALF, AND NEITHER ITS OWN.** They were watching the precondition *we* hold (our vocabulary growing) and had nothing on the one *they* hold (their export filter); we had the mirror image. Both discovered the same day, from opposite directions. ⚠️ **Measured before fixing: inserting `wine/dessert` passed the contract gate (exit 0) and the full schema-invariant suite (133/133)** — the only guard was prose on the `CategoryStyle` model. **Being able to see the other side's tripwire is not coverage.** Theirs fires when it *sees* the growth — after we ship it; ours fires before.

  ⚠️ **Their tripwire remains a BACKSTOP WITH A PRECONDITION — not automation you can lean on.** Built 2026-08-11 after we flagged that this item was prose while every other invariant here had ended up behind a check. **They pin OUR `category_styles` vocabulary**, and the detecting half re-derives it from a clone of this repo — so it fires only when someone has a clone to hand.
  ...
  🔒 **ASSERT A RELATIONSHIP THE DATABASE MAINTAINS, NOT A NUMBER A HUMAN MAINTAINS.** The maintenance side's rule, from finding their reachability audit used a hand-written probe list — so a constraint added tomorrow would never be probed while the audit printed "ALL n PROBES REFUSED". They replaced it with a query asking `pg_constraint` whether any live CHECK is unprobed.

  ⚠️ **The identical shape was live in this gate, and applying their rule found it.** `TABLES` is hand-written, so a *new catalog table* was not merely unwatched — it was unwatched while the gate printed a confident `ok`. **Measured: adding a sixth table with a rejection-shaped CHECK (`product_awards`, `CHECK (score BETWEEN 0 AND 100)`) left the gate reporting "ok 22 catalog CHECK constraints", exit 0.** Nothing anywhere said a contract-shaped table had appeared.

  ✅ Fixed by asking the database which tables reference the catalog spine and reporting any that are not watched (`UNWATCHED-CATALOG-TABLE`). ⚠️ **Pure FK-reachability was tried and rejected as the *definition***: it finds the new table correctly but also drags in `wines`, which is deliberately out of scope. So the curated list stays — it encodes a judgement the database cannot make — and the check makes an **addition** to that surface loud rather than silent. **The list is still authored; it is no longer trusted.** That distinction is the transferable part.
...
⚠️ **Collaborators are WRITABLE BUT INVISIBLE as built** — the write path is complete and validated, but all THREE application reads are lead-only: the two `lib/catalogSearch.ts` joins, plus `lib/catalogAddFlow.ts` branch 2, which returns the lead's id when adding a vintage to an existing product. Making collaborators readable needs a decision at each, and any new surface must resolve producer merge tombstones. **A merge proposal covers ALL of a producer's links regardless of role** (intent ruled 2026-08-18) — but 🔒 **it must be satisfied at READ time, through the effective-entity chain, NOT by re-pointing join rows**: § *Merge = pointer + lifecycle only* above forbids copying producer links into the survivor, and `lib/catalogSearch.ts` already relies on links staying with the loser. Resolving two links to one effective producer is a display de-duplication (lead beats collaborator), not a write. All recorded, with the standing agreement from the catalog-maintenance side and the ordering traps that would apply if write-time re-pointing were ever chosen instead, in the model-change proposal § 8b → *Collaborator links must resolve too*. Read that before building any multi-producer read surface.
...
- 🔒 **`producers.country` is the company's HOME country, not the origin of its wines** (definitional ruling from the catalog-maintenance side, 2026-08-18 — no schema change). A GB-based retailer brand carries GB while its wines are French. Two consequences for this policy: it must never be read as provenance in UI copy, and **name+country is a weak identity signal** — two producers identical on both can be different companies, which is precisely the residual the import contract hands to human review. ⚠️ **Verified against this tree (2026-08-18): `country` is not a matching or scoring input** — `lib/catalogSearch.ts` and `lib/catalogAddFlow.ts` select and display it, and the fold over `name` is what ranks. **Keep it that way.** Promoting it to a match key would make the GB-retailer case drive merges, and false merges are the expensive failure.
  - **On invalidation:** the data side flags that country values will be corrected via ordinary import updates as curation improves, and asks that a country change invalidate any cached merge-eligibility judgment. **No such cache exists** — suggestions are computed per query, and there is no persisted eligibility verdict at producer grain (the only durable verdict in the design is the EAN-conflict one, keyed on `(ean, incomingProductId)`). So there is nothing to invalidate today, and this is recorded as a **constraint on phase 3**: if the review queue ever memoises an eligibility judgment, the cache key must include every fact the judgment consumed, or the invalidation problem is created rather than solved. The cheap correct default is to keep recomputing.
...
- **Fact rules — enforced server-side at apply time**, not delegated to the caller's read-before-write (otherwise a curator edit landing between the caller's read and write is silently reverted):
  - **`status` and `linksTo` are never accepted from the caller and never updated by import.** On insert the **server itself** assigns `status = 'confirmed'` and `linksTo = null`; on update neither field is touched. ("Lives only in the INSERT arm" described where the value is written, which read as though the caller supplied it — the point is that the value is server-assigned.) Lifecycle transitions are staff actions only.
    ...
    🔒 **RULED 2026-08-18 — merge tombstones do NOT relax this; single write-authority over `status`/`linksTo` stands.** The catalog-maintenance side curates duplicate producers on their own side, and from the first post-fill cycle an entity we already imported may be re-delivered as a merge tombstone (`status='linked'`, `linksTo` → survivor). They proposed accepting it as a normal update ("id is stable; only status/linksTo change") and, on our pushback, **withdrew that framing and agreed to the shape below.**

    **Why the "cheap update" framing had to be refused.** `linksTo` is our merge pointer, and merge/unmerge is defined (§ Merge = pointer + lifecycle only) as an atomic transaction that also writes `catalog_audit` with the loser's `priorStatus` — which is the thing that makes unmerge correct. A caller writing the pointer directly either bypasses that audit row or forces import to synthesise one for an action no staff member took. The second reason is sharper: imported producers are explicitly **not final** (see the implementation plan § Phase 4 — two producers identical by name and country can be different companies), so our own review queue will be merging the same rows. Two independent merge authorities over one pointer, with no conflict rule, is the actual failure mode being avoided — not merely a bookkeeping preference.

    **The agreed shape.** A tombstone delivery is an **upstream lifecycle proposal, review-flagged, never auto-applied** — structurally the same class as the retraction record already specified in this section, and it reuses the EAN-conflict `deferred` / terminal-verdict machinery (implementation plan § EAN conflict → deferred) rather than inventing a merge-specific ACK:
    - The ACK outcome is a **third state**, not applied-vs-error: received, not applied, do not advance the journal for this entity. The data side confirmed this fits their journal semantics as-is (they advance only on applied-ACK), so an unreviewed proposal simply re-sends.
    - Staff resolution is a real merge through the normal path, writing `catalog_audit` in its own transaction — so the merge is ours, and the audit trail they asked for falls out of it.
    - A **rejection is a durable terminal verdict** carrying a **reason** (their request 2a — it feeds their curation record), returned on retry as an acknowledged rejection distinct from `deferred`, so the caller advances instead of looping forever. Same asymmetry as the EAN case: rejecting changes no data, so without a stored verdict the retry never terminates.

    ⚠️ **One requested refinement is NOT adoptable as stated, and is deferred to phase 3.** They asked (2b) that terminal verdicts bind **the proposal, not the pair**, so new evidence can produce a fresh proposal. The reused EAN machinery binds verdicts to a **pair** — `(ean, incomingProductId)` — and **no proposal identity exists anywhere** in the schema or `lib/` (verified 2026-08-18; phase 3's "change proposals" is a listed deliverable with no identity model behind it). Proposal-bound verdicts therefore require minting durable proposal ids with their own lifecycle, which is phase-3 review-queue design, not a phase-4 contract detail. **Interim:** verdicts bind the pair `(entityType, loserId, survivorId)`, and a staff member can clear a verdict.

    ⚠️ **CORRECTED 2026-08-18 — the manual clear is NECESSARY BUT NOT SUFFICIENT, so this deferral needed a different justification than the one first written here.** The original text said a staff clear "covers the case at the cost of a manual step". It does not close the loop, for a reason that is invisible from our side: **clearing a verdict changes nothing the sending side can observe.** A rejection advances the product's content hash (their push-diff base), so unchanged source data then produces no diff and the product is never re-sent; and while the verdict stands, a re-proposal is a no-op *by definition*, with no signal that it was lifted. The clear must be **delivered** to them — which is precisely what a `verdict-cleared` event on the change journal is for, and that operation is agreed-in-design, unbuilt, on a phase-4 table with phase 4 unstarted (§ below). **So the trap is currently open on both sides**, and `verdict-cleared` is what closes it — not the manual clear on its own.

    **The deferral still holds, on the honest reason:** proposal-bound verdicts require minting durable proposal ids with their own lifecycle, which is phase-3 review-queue design either way. What changes is the interim's cost — it is not "a manual step" but "no reconsideration path that the sender can act on until `verdict-cleared` ships". Settle the proposal-identity question when the review queue is designed; do not let phase 4 mint a second identity concept for it.

    ⚠️ **Symmetry, stated plainly in a section whose subject is specs not reading as implementations:** neither side is describing running code here. "They never re-send an unchanged product" is their *specified* behaviour — the exporter that would re-send on a diff is unbuilt on their side too.

    🔒 **CONSTRAINT ON PHASE 3, accepted 2026-08-18: if proposal identity is later minted, existing pair-bound verdicts MUST remain valid.** The data side accepted the deferral on that condition — they build their delivery leg against pair-bound semantics now, and a phase-3 upgrade must not require a migration on their side. So proposal identity, if it happens, is **additive over** `(entityType, loserId, survivorId)`, never a replacement that orphans verdicts already recorded under the pair. ⚠️ Read this as a real constraint on the phase-3 design space, not a courtesy: it was agreed in correspondence only, and the whole reason the authority split is written down here is that chat-only agreements get relitigated from one-sided records. Their contract documentation mirrors this section's authority split verbatim for the same reason.

    🔒 **CONSTRAINT ON PHASE 3, accepted 2026-08-18: proposal identity must be REFERENCEABLE BY THE SENDER, because a merge proposal needs a WITHDRAWAL operation.** The contract as agreed covers one direction only — a staff rejection is a durable terminal verdict returned with a reason. There is no reverse operation: **the sender cannot retract a proposal it has already made.** An identity decision can legitimately be revised or withdrawn after sending and before review, and today such a proposal sits in the queue as a live assertion nobody stands behind, with no way for staff to learn it was dropped. This compounds the silent-degradation problem in the latency note below — a stale queue is worse when some of its entries are already retracted.
        - **Why it is cheap now and awkward later:** verdicts are pair-bound (above), so **a withdrawal has nothing to name**. Phase 3 is already scoping proposal identity; the constraint is only that whatever identity it mints be exposed to the sender rather than kept internal to the queue. Note this composes with — does not contradict — the additive-over-the-pair constraint above: pair-bound verdicts stay valid, and the proposal id is what withdrawal addresses.
        - **Semantics against the terminal states.** Their three (adopted): `pending` → remove from the queue; **already reviewed (`accepted`/`rejected`) → acknowledged no-op, the staff decision stands** (a withdrawal never rescinds a merge that already happened); **unknown identity → explicit error, never silently accepted** — meaning *no proposal row exists*, never a row in a terminal state. That last one matters for the same reason the import parser refuses a malformed array instead of filtering it: silently accepting an unknown id turns a sender-side bug into invisible drift. Two more the lifecycle requires, since the status set is five-valued: **already `withdrawn` → acknowledged no-op** (which is what makes withdrawal RETRY-SAFE when a response is lost — without it a retried withdrawal would surface as an unknown-identity error and look like a sender bug); **`superseded` → acknowledged no-op**, no longer actionable.
        - 🔒 **Withdrawal and staff review must be SERIALIZED — exactly one transition wins.** Both `pending → withdrawn` and `pending → accepted/rejected` are live at the same time, and the docs above define the outcomes without saying who wins a race. Unserialized, a withdrawal can mark a proposal `withdrawn` while the review transaction is applying its merge — leaving a retracted proposal that nonetheless changed the catalog. Phase 3 must make the transition conditional on the current status under a row lock, with the entry mutation and the `accepted`/`rejected` status committed **in the same transaction** (§ *Resolution is one transaction* in the model-change proposal already requires the second half).
        - 🔒 **The two losing responses are NOT the same, and treating them symmetrically is the bug.** Which party lost decides what they must be told:
            - **Withdrawal loses** (review already resolved it) → the *sender* gets the acknowledged no-op above. Correct: nothing they believed is now false, the merge simply happened first.
            - **Staff review loses** (withdrawal won) → the *reviewer* must get an explicit **"no longer pending — withdrawn"** result. ⚠️ **Never apparent success.** A curator who clicks Apply and sees a success response will believe the merge landed; it did not, and they will not go back to check. Silent no-op here means staff carry a false belief about the catalog's state — the same silent-divergence class as an unreviewed queue, but with a human confidently on the wrong side of it. The result is surfaced **in the reviewer's current interaction**, so the outcome is legible rather than inferred from an absence — it does **not** put the proposal back on the actionable queue.
        - 🔒 **Three things, kept distinct** — conflating them is what makes "remove from the queue" and "tell the reviewer" look contradictory:
            1. **The actionable queue** — a `withdrawn` proposal leaves it, and never returns. This is what "remove from the queue" means, above.
            2. **The row** — retained, in its terminal `withdrawn` state, for audit/history. Never deleted (§ the model-change proposal's status lifecycle).
            3. **The in-flight reviewer's response** — an explicit withdrawn result, shown once, in the interaction they already had open. Not a queue entry, not a new work item.
        - 🔒 **`withdrawn` IS TERMINAL ON THE PROPOSAL ID, NEVER ON THE PAIR** — stated outright because the surrounding text is pair-bound and the misreading is a silent failure, not an error. **A pair whose proposal is withdrawn can legitimately be re-proposed later on fresh evidence**, and that second proposal must be reviewable on its own merits. Were `withdrawn` terminal on `(entityType, loserId, survivorId)`, every future proposal for that pair would return an acknowledged no-op forever, and nothing would surface the loss. ⚠️ **The EAN machinery shows the shape of this trap**: `rejected` is durable on `(ean, incomingProductId)`, so re-proposing the same pairing is a no-op rather than a fresh review item — and **not recoverable by the sender at all today**. A staff member can clear the verdict, but that is necessary and not sufficient: the clear changes nothing the sending side can observe (see the correction above), so reconsideration additionally requires the `verdict-cleared` delivery — agreed-in-design and unbuilt, per the status immediately below. ⚠️ Do not read the manual clear as an escape hatch on its own; it is half of one. **Per-proposal scoping means withdrawal never needs either half** — which is the whole argument for it. 🔒 **Status of `verdict-cleared`: AGREED IN DESIGN ONLY — not implemented, not deployed** (verified 2026-08-18 against the tree, at the maintenance side's request; it corresponds to their §prod_pull open item, and their entry stays OPEN). No verdict machinery exists in `lib/` or `app/`, `product_eans` has no application writer at all (import-owned, § *Catalog write ownership*), and the unified change journal that would carry the operation is a **phase-4 structural table** — phase 4 is unstarted. The composite keying they proposed is agreed and recorded; nothing more. ⚠️ Do not cite the presence of a spec, a migration, or a merged file as evidence that this is live — that substitution has produced a false cross-system closure in this repo before. Do not "simplify" withdrawal onto the pair key for consistency with the verdicts beside it.
        - ⚠️ **A withdrawn proposal is a fourth terminal state**, not a delete — the § *Change proposals are their own table* status lifecycle (`pending`/`accepted`/`rejected`/`superseded`) needs a `withdrawn` member, and the audit trail should show that a proposal existed and was retracted. Deleting the row would lose the fact that an assertion was made, which is the same reasoning that makes merges tombstones rather than deletes.
        - ✅ **Nothing is blocked today**: their sending side structurally refuses to propose a merge of any exported entity until the phase-4 endpoint exists, so no proposal can currently be stranded. This is a design constraint on phase 3, not a defect to fix now.

    ⏳ **Review-latency expectation: no number given, and none owed until phase 3 is scoped** (their 2c). An unreviewed proposal is deliberate divergence — their framing, and it is correct. This is a curation-capacity commitment rather than a schema question, and it interacts with the implementation plan's note that producer merges are a *steady* workload, not an occasional one. ✅ **Resolved for now by their side assuming the worst case:** they build nothing that depends on timely review, and treat divergence as **unbounded in duration**. That removes the deadline pressure, and it also means the queue can be sized on curation reality rather than on a promise already made to them. ⚠️ The flip side is that **nothing external will signal a backlog** — no caller timeout, no failing ACK — so an unreviewed merge queue degrades silently. Sizing it is a phase-3 obligation to ourselves, not to them.

    ✅ **The gap window is closed from their side — structurally, and as a stated undertaking.** Their curation tooling refuses to merge any entity we have already received until this proposal path exists (such merges queue on their side), and they have separately committed to performing **no hand-deletions or hand-merges of already-delivered entities** during the window between the fence opening and phase 4 (§ Seed + the truncate fence). So `docs/dev/catalog-manual-deletions.md` should record nothing originating from them. ⚠️ **That does NOT retire the file or soften the gate**: it still carries deletions *we* perform by hand — the case the FK posture cannot stop, since a product referenced only by its own `product_producers` join rows deletes successfully — and seeding it into the purge ledger remains a hard precondition on phase 4 accepting any import session or creating the initial baseline. An empty file is a state to verify at that point, never an assumption to skip the step on.

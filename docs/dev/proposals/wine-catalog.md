...
⚠️ **Collaborators are WRITABLE BUT INVISIBLE as built** — the write path is complete and validated, but all THREE application reads are lead-only: the two `lib/catalogSearch.ts` joins, plus `lib/catalogAddFlow.ts` branch 2, which returns the lead's id when adding a vintage to an existing product. Making collaborators readable needs a decision at each, and any new surface must resolve producer merge tombstones. **A merge proposal covers ALL of a producer's links regardless of role** (ruled 2026-08-18) — where re-pointing would collide with the `(product_id, producer_id)` PK, collapse to one row keeping the stronger role (lead beats collaborator); two leads on one product is unrepresentable, so the one-lead invariant survives every case, and link cardinality legitimately shrinks across a merge. All recorded, with the standing agreement from the catalog-maintenance side, in the model-change proposal § 8b → *Collaborator links must resolve too*. Read that before building any multi-producer read surface.

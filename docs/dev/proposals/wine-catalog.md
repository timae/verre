...
⚠️ **Collaborators are WRITABLE BUT INVISIBLE as built** — the write path is complete and validated, but all THREE application reads are lead-only: the two `lib/catalogSearch.ts` joins, plus `lib/catalogAddFlow.ts` branch 2, which returns the lead's id when adding a vintage to an existing product. Making collaborators readable needs a decision at each, any new surface must resolve producer merge tombstones, and the merge-proposal contract must say explicitly whether a proposal covers collaborator links. All recorded, with the standing agreement from the catalog-maintenance side, in the model-change proposal § 8b → *Collaborator links must resolve too*. Read that before building any multi-producer read surface.

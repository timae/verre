...
- `ATTRIBUTIONS_PATH` (optional) — absolute path to a JSON file overriding the bundled legal attribution entries (`lib/attributions.ts`). Unset ⇒ the entries committed in that module are used, which is the normal case. 🔒 **This is a LICENCE-OBLIGATION surface**, so the loader **falls back to the bundled entries** on any failure (missing file, bad JSON, schema mismatch, empty list) and logs loudly — failing closed would render a blank legal page and breach the very licences it exists to satisfy. Exists so adding or correcting a source is an **ops change, not a release**, since licence terms change independently of our deploy cycle. Deliberately a file rather than an env var: `licence.text` is a multi-line verbatim legal block (the MIT notice is 1072 bytes with hard newlines) and shell quoting would silently corrupt a string we are legally required to reproduce exactly. The value is process-cached after first read. ⚠️ **If you override this, the native app's bundled fallback snapshot no longer matches** — the CI parity gate only compares the two in-repo copies, so an override that changes legal wording must be paired with an app release, or offline users will see the superseded text.

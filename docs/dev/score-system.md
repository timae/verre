...
Both `ratings.score` (in-session) and `checkins.score` (standalone) are `Decimal(3,2)` in Postgres. The same value lives transiently in Redis as a JSON number under `s:{CODE}:r:{IDENTITYID}:{WINEID}` while a session is live, then gets archived to the relational column on commit.

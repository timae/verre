...
Both payload kinds carry the rating's `aromas` (`[{a, m, p?}]`, aroma-layer.md §1) — shipped UNREDACTED even on a blind wine, like `score`/`flavors`: descriptor selections are the taster's own perception, never wine identity (§7). The mobile feed impression detail renders them via `AromaReadChips`; web render is PR D.

...
| POST | /api/checkins | Create a standalone check-in. Body: `{wineName, producer?, vintage?, grape?, type?, wineRegion?, wineCountry?, vinification?, description?, purchaseUrl?, productId?, vintageId?, score?, flavors?, aromas?, notes?, imageData?, venueName?, city?, country?, lat?, lng?, taggedUserIds?, copyFromCheckinId?}`. `productId`/`vintageId` link the minted wine to the shared catalog and are validated server-side (`vintageId` requires `productId`). `wineRegion`/`wineCountry` are the wine's origin (`country` is the venue's); the metadata fields land on the minted wine row. `flavors` is the same `{structure-axis: 0–5}` map as `/rate` (keys outside the type's axis set are rejected). `aromas` is the same `[{a, m, p?}]` descriptor list as `/rate` (unknown node / disallowed modifier → 400); omitted = `[]`. See `docs/dev/social-feed.md`. |
| PATCH | /api/checkins/:id | Edit own feed post. `:id` is a `feed_items.id`. Standalone kind: partial body `{wineName?, producer?, vintage?, grape?, type?, wineRegion?, wineCountry?, vinification?, description?, purchaseUrl?, productId?, vintageId?, score?, flavors?, aromas?, notes?, imageData?, venueName?, city?, country?, lat?, lng?, taggedUserIds?}` — `aromas` is present-replaces / omitted-preserves (a present `[]` clears, an omitted field keeps stored selections); image replace reclaims old S3. An edit that changes the wine's identity (`wineName`/`producer`/`vintage`) clears the catalog link unless `productId` re-links explicitly; cosmetic edits keep it. Session kind: rating-only body `{wineId, score?, flavors?, aromas?, notes?}` against the caller's own rating (fields outside those five are ignored, not rejected); an edit that empties the rating runs the engagement-deletion cascade. See `docs/dev/social-feed.md`. |
...
**Wine catalog** — the shared producer/product/vintage reference data. Both endpoints are **staff-only until the catalog is publicly released** (`CATALOG_PUBLIC_ENABLED`, see `docs/dev/deployment.md`); everyone else gets 404. A wine is linked to the catalog by passing `productId`/`vintageId` to the wine and check-in endpoints above.

| Method | Path | Description |
|--------|------|-------------|
| GET | /api/catalog/search | Suggestions — `?kind=producer&q=` or `?kind=product&q=&producerId=`. Returns likely matches for a person to choose from; picking one is always an explicit choice, never automatic. 30/min |
| POST | /api/catalog/entries | Create catalog entries for a wine being added. Body `{producerId?, producer?, product, year?, collaboratorIds?}` — supply `producerId` for an existing maker or `producer` to create one. `year` absent = link at product level (year unknown), `null` = the non-vintage bottling, a number = that vintage. Everything created here is provisional and enters the review queue. 60/h |
